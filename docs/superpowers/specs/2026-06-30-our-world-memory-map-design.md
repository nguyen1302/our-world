...
- World Map (Leaflet + OSM), marker cluster, Memory Card.
- Timeline drill-down Year→Month→Day→TimeCluster→Memory, đồng bộ 2 chiều với map.
- Scratch Map (tô màu quốc gia đã có Memory).
- Statistics (đếm Memory/ảnh/thành phố/quốc gia).
- On This Day.
- Title Memory **rule-based** (địa điểm + ngày), user sửa được.
...
- **World Map:** Leaflet + OSM tiles; `leaflet.markercluster` gom marker; click marker → **Memory Card** (cover, gallery, ngày, địa điểm, ghi chú).
...
- **Scratch Map:** layer GeoJSON, tô màu quốc gia đã có ≥1 Memory; "khám phá" dần qua năm tháng (hiện tại mới có Việt Nam).
- **Statistics:** đếm Memory, ảnh, (video=0 ở v1), thành phố, quốc gia.
- **On This Day:** Memory trùng ngày-tháng hôm nay được highlight ("30 June — 2 years ago — You were here.").

## 10. Reverse geocoding

- **Nominatim (OpenStreetMap)** — free, cần tôn trọng usage policy (rate limit, User-Agent).
